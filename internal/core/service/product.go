...
func (s *service) CatalogProductQuery(ctx context.Context, cityName string, query string) ([]*dproduct.ProductCatalogQuerySearchResult, error) {
	ctx, span := otel.Tracer("service").Start(ctx, "service.CatalogProductQuery")
	defer span.End()
	return s.repo.CatalogProductQuery(ctx, cityName, query)
}
